...
# ─────────────────────────────────────────────
# 3. الإعدادات الخاصة بأداة الأقوال والحكم
# ─────────────────────────────────────────────
BASE_DIR         = os.path.dirname(os.path.abspath(__file__))
STATE_FILE       = os.path.join(BASE_DIR, "quotes_state.json") # ملف حالة منفصل للأداة الجديدة
TARGET_CHANNEL   = "@your_quotes_channel" # اسم قناة الأقوال والحكم هنا
MAX_UPLOADS      = int(os.getenv("MAX_UPLOADS", "1000"))
GDRIVE_SCOPES    = ["https://www.googleapis.com/auth/drive"]

# ─────────────────────────────────────────────
# 4. دالة الاتصال بـ OpenRouter (مخصصة للتصنيف)
# ─────────────────────────────────────────────
def process_quote_with_openrouter(text: str, retries: int = 3, delay: int = 5) -> dict:
    ...
    system_prompt = (
        ...
        "1. حدد ما إذا كان النص يحتوي على حكمة، قول مأثور، مثل شعبي، أو اقتباس مفيد. إذا كان مجرد إعلان أو كلام عشوائي، اجعل 'valid' يساوي false.\n"
        "2. صنف القول إلى فئة رئيسية واحدة تناسبه لتكون اسم الملف (مثل: 'حكم وأقوال'، 'أمثال شعبية'، 'تطوير الذات'، 'اقتباسات كتب'). اجعل اسم الفئة بدون رموز خاصة.\n"
        "3. نسق القول بصيغة Markdown ليكون مناسباً كسطر مضاف في ملف، استخدم التنسيق التالي:\n"
        "- > \"القول هنا\" — **اسم القائل إن وجد** #وسم_الفئة\n\n"
        "يجب أن يكون ردك بصيغة JSON حصراً وبدون أي نص إضافي:\n"
        "{\n"
        "  \"valid\": true,\n"
        "  \"category\": \"اسم الفئة هنا (يصلح كاسم ملف)\",\n"
        "  \"formatted_quote\": \"السطر المنسق هنا\"\n"
        "}\n"
        "لا تضف ```json. الرد يبدأ بـ { وينتهي بـ } فقط."
    )
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            log.warning(f"⚠️ محاولة {attempt} فشلت في تحليل رد الذكاء الاصطناعي: {e}")
            time.sleep(delay)
...
def get_file_content(service, file_id: str) -> str:
    """تحميل محتوى الملف الحالي من الجوجل درايف."""
...
def append_quote_to_file(category: str, formatted_quote: str) -> bool:
    """تقوم بالبحث عن ملف الفئة، تحديثه بإضافة السطر الجديد، أو إنشائه إن لم يوجد."""
    try:
        ...
        if file_id:
            # الملف موجود: نقرأ المحتوى القديم ونضيف الجديد
            current_content = get_file_content(service, file_id)
            # التأكد من وجود سطر جديد قبل الإضافة
            ...
            log.info(f"🔄 تم تحديث وإضافة القول إلى الملف القائم: '{filename}'")
        else:
            # الملف غير موجود: ننشئه بعنوان الفئة ونضع القول كأول سطر
...
async def main():
    ...
    try:
        ...
        log.info(f"🎯 متصل بقناة الأقوال: {entity.title}")
    except Exception as e:
        log.error(f"❌ تعذّر الوصول للقناة: {e}"); await client.disconnect(); return
    ...
    try:
        ...
        for msg in messages:
            ...
            if ai_result.get("valid") and ai_result.get("category"):
                ...
                log.info(f"skip ⏭️ رسالة رقم {msg.id} (ليست قولاً مفيداً أو إعلان).")
